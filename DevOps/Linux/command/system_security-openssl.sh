...
运行模式：
    交互模式    :输入openssl回车进入交互模式
    批处理模式  :输入带命令选项的openssl进入批处理模式

功能组成部分：  // 其目录结构也是围绕这三个功能部分进行规划的。 
    密码算法库
    SSL协议库
    应用程序
...
对称加密算法    加密解密使用同一个密码
    分组加密算法
        ...
        7种方法都支持电子密码本模式（ECB）、加密分组链接模式（CBC）、加密反馈模式（CFB）和输出反馈模式（OFB）四种常用的分组密码加密模式。
...
非对称加密算法
    DH算法          一般用户密钥交换
    RSA算法         以用于密钥交换，也可以用于数字签名，当然，如果你能够忍受其缓慢的速度，那么也可以用于数据加密。
    DSA算法         一般只用于数字签名
    椭圆曲线算法（EC） 
...
信息摘要算法
    ...
    MD5
    MDC2
    SHA（SHA1）     SHA算法事实上包括了SHA和SHA1两种信息摘要算法
    RIPEMD
...
公钥加密:   密码成对出现 公钥/私钥
单向加密，散列加密:提取数据特征码,常用于校验数据完整性
    1、雪崩效应:文件稍有更改 特征码即更改
    2、定长输出:特征码长度输出固定
        md5:    128位   (Message Digest)
        SHA1:    160位   (Secure Hash Algorithm)
        SHA256:
    获取特征码: # md5sum filename
